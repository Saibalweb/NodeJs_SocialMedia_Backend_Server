...
const followUser = asyncHandler(async (req, res) => {
  ...
  const alreadyFollowed = await Connection.findOne({
  ...
  res
    .status(202)
    .json(
      new ApiResponse(202, { response: connection }, "Successefully Followed!"),
    );
});
const unFollowUser = asyncHandler(async (req, res) => {
  ...
  if (!alreadyFollowed) {
    throw new ApiError(
      404,
      "You cannot unfollow this user as You are not following this user!",
    );
  }
  ...
  res
    .status(202)
    .json(
      new ApiResponse(202, { response: unfollow }, "Successfully Unfollowed!"),
    );
});
const getFollowings = asyncHandler(async (req, res) => {
  ...
  const followings = await Connection.aggregate([
    {
      $match: {
        followerId: user?._id,
      },
    ...
    },
    {
      $project: {
        _id: 1,
        followingId: 1,
        "followingUserData.name": 1,
        "followingUserData.avatar": 1,
        "followingUserData.userName": 1,
      },
    },
  ]);
  ...
  res
    ...
    .json(
      new ApiResponse(
        202,
        { response: { followings, followingsCount } },
        "Successfully fetched followings",
      ),
    );
});
    ...
    const followers = await Connection.aggregate([
        ...
        },
        {
            $project:{
                _id:1,
                follwerId:1,
                "followerUserData.name":1,
                "followerUserData.userName":1,
                "followerUserData.avatar":1
            }
        }
    ])
  ...
  res
    ...
    .json(
      new ApiResponse(
        ...
        "Fetched Followers Successfully!",
      ),
    );
});
